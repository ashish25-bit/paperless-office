...
User.prototype = {
    ...
    // putting the message from editor page in the message table
    put_doc_msg: (info, questionMarks, callback) => {
        let query = `INSERT INTO messages (room, message, sent, date, time, type) VALUES ${questionMarks}`
        db.query(query, info, (err, res) => {
            if (err) return callback('Server Error')
